...
<section class="pt-10">
  <section class="h-screen shadow-md">
    <form action="{{ route('koor.update_pengajuan_ortu', $data->id) }}" method="POST" class="container bg-white w-full mx-auto md:w-3/4 grid grid-cols-2 p-8 gap-8"  enctype="multipart/form-data">
        @csrf
        <div>
        ...
        <div class="">
            ...
            <div class=" relative pb-4">
                <input type="text" id="user-info-email" name="job" class=" rounded-lg border-1 flex-1 appearance-none border border-gray-300 w-full py-2 px-4 bg-white text-gray-700 placeholder-gray-400 shadow-sm text-base focus:outline-none focus:ring-2 focus:ring-purple-600 focus:border-transparent" placeholder="pekerjaan" value={{ $data->job }}>
                @error('job')
                <div class="bg-red-200 border-red-600 text-red-600 border-l-4 p-4" role="alert">
                    <p>
                        {{ $message }}
            ...
            <div class=" relative pb-4">
                <select
                    data-te-select-init
                    data-te-select-placeholder="Example placeholder"
                    class="rounded-lg border-1 flex-1 appearance-none border border-gray-300 w-full py-2 px-4 bg-white text-gray-700 placeholder-gray-400 shadow-sm text-base focus:outline-none focus:ring-2 focus:ring-purple-600 focus:border-transparent"
                    name="salary"
                >
                    <option value="0 - 1.000.000" {{ old('salary') == '0 - 1.000.000' ? 'selected' : '' }}>0 - 1.000.000</option>
                    <option value="1.000.000 - 3.000.000" {{ old('salary') == '1.000.000 - 3.000.000' ? 'selected' : '' }}>1.000.000 - 3.000.000</option>
                    <option value="3.000.000 - 5.000.000" {{ old('salary') == '3.000.000 - 5.000.000' ? 'selected' : '' }}>3.000.000 - 5.000.000</option>
                    <option value="> 5.000.000" {{ old('salary') == '> 5.000.000' ? 'selected' : '' }}> > 5.000.000</option>
                </select>
            ...
            <div class=" relative pb-4">
                <input type="number" id="user-info-email" name="phone" class=" rounded-lg border-1 flex-1 appearance-none border border-gray-300 w-full py-2 px-4 bg-white text-gray-700 placeholder-gray-400 shadow-sm text-base focus:outline-none focus:ring-2 focus:ring-purple-600 focus:border-transparent" placeholder="nomor hp" value={{ $data->phone }}>
                @error('phone')
                <div class="bg-red-200 border-red-600 text-red-600 border-l-4 p-4" role="alert">
                    <p>
                        {{ $message }}
